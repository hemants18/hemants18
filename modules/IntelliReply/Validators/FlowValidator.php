...
class FlowValidator
{
    ...
    private function validateButtons(array $data)
    {
        ...
        if (isset($data['buttonType']) && $data['buttonType'] === 'buttons') {
            // Check if at least one button (button1, button2, button3) has a value
            $hasValue = false;
            foreach (['button1', 'button2', 'button3'] as $buttonKey) {
                if (!empty($data['buttons'][$buttonKey])) {
                    $hasValue = true;
                    break;
                }
            }

            // If no button has a value, return an error message
            if (!$hasValue) {
