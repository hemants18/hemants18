<script setup>
    ...
    const deleteAction = (key) => {
        form.delete('/admin/blog/authors/' + key);
    }
    ...
    <Table :rows="rows">
        <TableHeader>
            <TableHeaderRow>
                ...
                <TableHeaderRowItem>{{ $t('Created AT') }}</TableHeaderRowItem>
                <TableHeaderRowItem :position="'last'"></TableHeaderRowItem>
            </TableHeaderRow>
        </TableHeader>
        <TableBody>
            <TableBodyRow v-for="(item, index) in rows.data" :key="index">
                <TableBodyRowItem :position="'first'">{{ item.full_name }}</TableBodyRowItem>
                
                <TableBodyRowItem>{{ $t(item.created_at) }}</TableBodyRowItem>
