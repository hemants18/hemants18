<script setup>
    ...
    const emit = defineEmits(['delete','verify']);
    ...
    <Table :rows="rows">
        <TableHeader>
            <TableHeaderRow>
                ...
                <!-- <TableHeaderRowItem>
                    <span class="float-right">{{ $t('Whatsapp No Verify') }}</span>
                </TableHeaderRowItem> -->
        ...
        <TableBody>
            <TableBodyRow v-for="(item, index) in rows.data" :key="index" :class="!isLastRow(index) ? 'border-b' : ''">
                ...
                <!-- <TableBodyRowItem class="capitalize">
                    <span class="float-right py-1 rounded-[5px] text-xs px-3 bg-[#ddebf7] text-slate-700">{{ statusLabel(item.
        whatsapp_verify) }}</span>
                </TableBodyRowItem> -->
                    ...
                    <Dropdown v-if="item.role != 'admin'" :align="'right'" class="mt-2">
                        ...
                        <template #items>
                            <DropdownItemGroup>
                                ...
                                <!-- <DropdownItem  as="button" @click="verify(item.id)">{{ $t('Verify') }}</DropdownItem> -->
