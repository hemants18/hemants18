    ...
    <div>
        <div class="bg-slate-100 md:bg-slate-50 rounded-[0.5rem]">
            <table class="w-full">
                <TableHeader>
                    <TableHeaderRow>
                        <TableHeaderRowItem :position="'first'" class="pl-2 py-2 pb-2 ">{{ $t('Name') }}</TableHeaderRowItem>
                        <TableHeaderRowItem class="pl-2 py-2 pb-2">{{ $t('Color') }}</TableHeaderRowItem>
                        <TableHeaderRowItem class="pl-2 py-2 pb-2">{{ $t('Created') }}</TableHeaderRowItem>
                        <TableHeaderRowItem class="pl-2 py-2 pb-2">{{ $t('Usage') }}</TableHeaderRowItem>
                ...
                <draggable tag="tbody" :list="rows.data" handle=".handle" :clone="false" item-key="id">
                    <template #item="{ element, index }">
                        <tr class="hover:bg-slate-50 md:border-b-0 md:border-t border-[#d1d5db] text-sm">
                            ...
                            <td class="pl-2 py-2 pb-2">
                              <span
                                class="inline-block w-4 h-4 rounded-full mr-2 align-middle"
                                :style="{ backgroundColor: element.color }"
                              ></span>
                              <span class="align-middle text-sm text-slate-700">
                                {{ element.color }}
                              </span>
                            </td>
                            <td class="pl-2 py-2 pb-2 capitalize">{{ formatDate(element.created_at) }}</td>
                            <td class="pl-2 py-2 pb-2 capitalize">{{ element.contact_tag_count }}</td>
                            <td class="pr-4 float-right">
                                ...
                                <template #items>
                                    <DropdownItemGroup>
                                        <DropdownItem as="button" @click="edit(element.id)">{{ $t('Edit') }}</DropdownItem>
                                        <DropdownItem as="button" @click="openAlert(element.id)">{{ $t('Delete') }}</DropdownItem>
                                    </DropdownItemGroup>
                                </template>
                                </Dropdown>
                            </td>
