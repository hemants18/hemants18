            ...
            <div class="flex mt-4">
                <div class="md:w-1/5 bg-white border border-slate-200 h-[fit-content] rounded-lg pb-10 md:block hidden">
                    <ul class="pt-2 space-y-1 text-sm mb-2 px-2">
                        ...
                        <li class="hover:bg-slate-50 hover:text-black rounded-[5px] px-1 truncate cursor-pointer" :class="$page.url.startsWith('/settings/tags') ? 'bg-slate-50 text-black' : ''">
                            <Link rel="noopener noreferrer" href="/settings/tags" class="flex items-center p-2 space-x-3 rounded-md">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M7.0498 7.0498H7.0598M10.5118 3H7.8C6.11984 3 5.27976 3 4.63803 3.32698C4.07354 3.6146 3.6146 4.07354 3.32698 4.63803C3 5.27976 3 6.11984 3 7.8V10.5118C3 11.2455 3 11.6124 3.08289 11.9577C3.15638 12.2638 3.27759 12.5564 3.44208 12.8249C3.6276 13.1276 3.88703 13.387 4.40589 13.9059L9.10589 18.6059C10.2939 19.7939 10.888 20.388 11.5729 20.6105C12.1755 20.8063 12.8245 20.8063 13.4271 20.6105C14.112 20.388 14.7061 19.7939 15.8941 18.6059L18.6059 15.8941C19.7939 14.7061 20.388 14.112 20.6105 13.4271C20.8063 12.8245 20.8063 12.1755 20.6105 11.5729C20.388 10.888 19.7939 10.2939 18.6059 9.10589L13.9059 4.40589C13.387 3.88703 13.1276 3.6276 12.8249 3.44208C12.5564 3.27759 12.2638 3.15638 11.9577 3.08289C11.6124 3 11.2455 3 10.5118 3ZM7.5498 7.0498C7.5498 7.32595 7.32595 7.5498 7.0498 7.5498C6.77366 7.5498 6.5498 7.32595 6.5498 7.0498C6.5498 6.77366 6.77366 6.5498 7.0498 6.5498C7.32595 6.5498 7.5498 6.77366 7.5498 7.0498Z"fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/> </svg>
                                <span>{{ $t('Tag settings') }}</span>
                            </Link>
                        </li>
                        <li class="hover:bg-slate-50 hover:text-black rounded-[5px] px-1 truncate cursor-pointer" :class="$page.url.startsWith('/settings/optin') ? 'bg-slate-50 text-black' : ''">
                            <Link rel="noopener noreferrer" href="/settings/optin" class="flex items-center p-2 space-x-3 rounded-md">

                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M8 7C8.55228 7 9 7.44772 9 8V16C9 16.5523 8.55228 17 8 17C7.44772 17 7 16.5523 7 16V8C7 7.44772 7.44772 7 8 7Z" fill="#000000"></path> <path d="M17 16V8C17 7.44772 16.5523 7 16 7C15.4477 7 15 7.44772 15 8V16C15 16.5523 15.4477 17 16 17C16.5523 17 17 16.5523 17 16Z" fill="#000000"></path> <path fill-rule="evenodd" clip-rule="evenodd" d="M6.44444 2C3.98985 2 2 3.98985 2 6.44444V17.5556C2 20.0102 3.98985 22 6.44444 22H17.5556C20.0102 22 22 20.0102 22 17.5556V6.44444C22 3.98985 20.0102 2 17.5556 2H6.44444ZM17.5556 4H6.44444C5.09442 4 4 5.09442 4 6.44444V17.5556C4 18.9056 5.09442 20 6.44444 20H17.5556C18.9056 20 20 18.9056 20 17.5556V6.44444C20 5.09442 18.9056 4 17.5556 4Z" fill="#000000"></path> <path d="M16 14.75C16.9665 14.75 17.75 13.9665 17.75 13C17.75 12.0335 16.9665 11.25 16 11.25C15.0335 11.25 14.25 12.0335 14.25 13C14.25 13.9665 15.0335 14.75 16 14.75ZM8 12.75C8.9665 12.75 9.75 11.9665 9.75 11C9.75 10.0335 8.9665 9.25 8 9.25C7.0335 9.25 6.25 10.0335 6.25 11C6.25 11.9665 7.0335 12.75 8 12.75Z" fill="white" stroke="#000000" stroke-width="1.5"></path> </g></svg>
                                <span>{{ $t('Opt-In Setting') }}</span>
                            </Link>
                        </li> 
...
<script setup>
    ...
    const modules = computed(() => {
        const raw = page.props.modules
        if (!raw) return []

        // Inertia paginator support
    ...
    const moduleActive = (moduleName) => {
        return modules.value.some(
            module => module.name === moduleName && module.status === 1
        )
    }
</script>