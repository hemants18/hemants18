...
const emit = defineEmits(["data-sent","open"]);
...
function openNode() {
	// store.selectNode(localStates.value);
	emit("open", localStates.value);
}
</script>
<template>
		...
		<div
			...
			<div class="content">
				<div class="card" style="width: 18rem;">
					<div class="card-body">
						<p class="card-title"><i class="las la-walking"></i> {{ $t('start_bot_flow') }}</p>
						<div v-if="localStates.keyword" class="bg-body-tertiary">
						  <span>{{ $t('keyword') }} : {{ localStates.keyword }}</span>
						</div>
						<div v-if="localStates.matching_types" class="bg-body-tertiary">
						  <span> {{ $t('matching_type') }} : {{ localStates.matching_types ? localStates.matching_types : "exacts" }}</span>
						</div>
					</div>
				</div>
			</div>
