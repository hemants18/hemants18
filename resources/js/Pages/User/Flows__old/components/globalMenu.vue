...
import NodeModal from './FlowModal.vue'
...
const flowId = route.props.flowId
...
function saveFlow() {
	...
	if (nodes.length - edges.length > 1) {
		...
		toast($t('you_must_connect_all_nodes_to_export'), {
                autoClose: 3000,
            });
	...
	if (flowId == "create") {
		...
		url = getMixinValue.getUrl("flow-builders/" + flowId);
	}
...
reader.onload = (e) => {
	...
	toast(`File ${importedFileName.value} was uploaded, you can now import its content.`, {
                autoClose: 3000,
            });
...
const openModal = () => {
	...
	if (nodes.length - edges.length > 1) {
		toast('You must need to connect all nodes to proceed', {
                autoClose: 3000,
            });
		// toastr.error($t('you_must_need_to_connect_all_nodes_to_proceed'));
		return false;
	}
...
<template>
	<div>
		<div class="flowBuilder__left container rounded !w-[190px]" style="position: relative; width: ">
			<SidebarVue />
			<div class="borders">
      ...
      >{{ importedFileName || "Select file" }}
		...
		<NodeModal class="sp-modal" :isOpen="isModalOpened" @modal-close="closeModal" name="note-modal">
				<div class="row w-100">
						<div class="col-lg-6">
							<p class="m-0 mt-3">{{ flowId == "create" ? $t('add_flow') :
								$t('edit_flow') }}</p>
						</div>
				...
				<div class="modal-body">
						<div class="mb-4">
							<div class="title-mid mb-4">{{ $t('flow_title') }}</div>
							<input type="text" class="sp_modal_text" v-model="data.name" />
						</div>
						<div class="mb-4">
							<label for="labels" class="form-label">{{ $t('contact_lists') }}</label>
							<select class="form-control w-full mt-2" v-model="data.contact_list_id">
								<option value="" disabled>{{ $t('contact_lists') }}</option>
								<option v-for="(list, index) in data.contact_lists" :key="index" :value="list.id">{{
									list.full_name }}</option>
							</select>
						</div>
						<!-- <div class="mb-4">
							<label for="sagement" class="form-label">{{ $t('segments') }}</label>
