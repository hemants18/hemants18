<template>
  <div
    v-if="getValueByKey('logo') === null"
    class="flex items-center justify-between px-5 pt-5 h-20 mb-4"
  >
    <h2 v-if="!menuIconsOnly" class="ml-2 text-2xl">
      {{ getValueByKey("company_name") }}
    </h2>
  ...
  <div v-else class="flex items-center justify-between px-5 pt-5 h-20 mb-1">
    <Link v-if="menuIconsOnly === false" href="/dashboard">
      <img
        :src="'/media/' + getValueByKey('logo')"
        :alt="getValueByKey('company_name')"
    ...
    <Link v-else href="/dashboard">
      <img
        :src="'/media/' + getValueByKey('favicon')"
        :alt="getValueByKey('company_name')"
        class="w-32 object-contain h-full ps-2"
      />
    </Link>
  ...
  <div class="flex-grow space-y-3 px-2 overflow-y-scroll">
    <div class="flex-1">
      <ul class="pt-2 space-y-1 text-sm mb-2">
        <li
          class="hover:bg-primary-gradient hover:text-white rounded-[5px] px-2 truncate"
          :class="
        ...
        >
          ...
          >
            ...
            <span :class="menuIconsOnly ? 'hidden' : ''">{{
              $t("Dashboard")
            }}</span>
          </Link>
        </li>
        ...
        >
          ...
          >
            ...
            <span
              ...
              class="!bg-white px-2 text-[11px] rounded-md !text-black"
              >{{ unreadMessages }}</span
            >
          </Link>
        </li>
        ...
        <li
          class="hover:bg-primary-gradient hover:text-white rounded-[5px] px-2 truncate"
          :class="
            $page.url.startsWith('/automation/basic') ||
            $page.url.startsWith('/automation/flows')
              ? 'bg-primary-gradient text-white'
              : ''
          "
        >
      ...
      <ul class="pb-4 space-y-1 text-sm mt-2">
        ...
        <li
          ...
          :class="
            ...
            $page.url.startsWith('/subscription')
              ? 'bg-primary-gradient text-white'
              : ''
          "
        >
  ...
  >
    <div class="flex space-x-1">
      <span>{{ $t("Team") }}:</span>
      <span class="text-ellipsis w-[120px] truncate">{{
  ...
  <div
    class="flex items-center m-3 p-2 rounded-xl h-14 py-1 md:py-1 mt-2 gap-x-4 bg-primary-gradient text-white"
    :class="!menuIconsOnly ? 'justify-between' : 'justify-center'"
  >
    <div v-if="!menuIconsOnly" class="flex space-x-2">
      <div class="rounded-xl p-1 bg-white">
        <img
          v-if="user.avatar"
        ...
        <div
          v-else
          class="rounded-full w-9 h-9 flex justify-center items-center text-black"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
      ...
      <div>
        <h2
          class="text-[15px] capitalize truncate w-[6em]"
        ...
        >
          <span
            @click="openModal"
            class="text-sm hover:underline dark:text-gray-400 cursor-pointer !text-white"
            >{{ $t("View profile") }}</span
          >
        </span>
      </div>
    </div>
    <Link href="/logout" class="w-[fit-content] p-2" @click.prevent="logout">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
  ...
  <Modal :label="$t('Switch teams')" :isOpen="isLocationSwitchModalOpen">
    <div class="mt-2 grid grid-cols-1 gap-x-6">
      <div class="pt-3 space-y-2 text-sm">
        ...
        >
          <div class="flex items-center gap-x-2">
            <span
              class="bg-slate-200 w-10 h-10 rounded-lg flex items-center justify-center"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
        ...
        <div
          @click="isOpenOrganizationModal = true"
      ...
      <div class="mt-4 border-t pt-4">
        <button
          type="button"
          @click.self="isLocationSwitchModalOpen = false"
...
import { leaveAgentPresence } from "@/agentPresence";
...
const logout = () => {
    const companyId = props.organization?.id;
    if (companyId) leaveAgentPresence(companyId);
    router.visit('/logout');
};
...
onUnmounted(() => {
    leaveAgentPresence(props.organization.id)
})
</script>
