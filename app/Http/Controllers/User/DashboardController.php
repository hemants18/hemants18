...
class DashboardController extends BaseController
{
    ...
    public function index(Request $request){
        ...
        $data['chatCount'] = Chat::where('organization_id', $organizationId)->whereNull('deleted_at')->whereHas('contact', function ($query) {
        ...
        $data['embeddedSignupActive'] = (int) $settings->get('is_embedded_signup_active', 0);
        // $data['embeddedSignupActive'] = CustomHelper::isModuleEnabled('Embedded Signup');

        //show qrcode
        $data['whatsappqrcode'] = '';
        $data['whatsappqrcodeurl'] = '';
        if( $config )
        {
            $whatsppNo = new PhoneNumber($config['whatsapp']['display_phone_number']);
            $whatsappNo = str_contains($whatsppNo, '+') ? str_replace('+', '', $whatsppNo) : $whatsppNo;
            $data['whatsappqrcodeurl'] = 'https://wa.me/'.$whatsappNo."?text=".config('whatsapp.qr_msg');
            $data['whatsappqrcode'] = CustomHelper::whatsappQR($data['whatsappqrcodeurl']);
        }
