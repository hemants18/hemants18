...
class FlowBuilderService
{
    ...
    public function getRows($uuid)
    {
        $organizationId = session()->get('current_organization');
        $model = Flow::where(['organization_id'=>$organizationId,'uuid'=>$uuid])->first();
        return $model;
    }
    ...
    public function uploadFile(object $request)
    {
        ...
        if($request->hasFile('file')){
            ...
            if($storage === 'local'){
                $file = Storage::disk('local')->put('public/flow_builder', $fileContent);
                $mediaFilePath = $file;

                $avatar = '/media/' . ltrim($mediaFilePath, '/');
            } else if($storage === 'aws') {
                $file = $request->file('file');
                $uploadedFile = $file->store('uploads/media/flow_builder/' . $this->organizationId, 's3');
                $mediaFilePath = Storage::disk('s3')->url($uploadedFile);

                $avatar = $mediaFilePath;
            }
        }
    ...
    { 
        ...
        try {
            ...
            // SubscriptionService::isSubscriptionFeatureLimitReached($this->organizationId, 'message_limit')
            ...
            return response()->json([
                ...
                'redirect_url' => route('flowsbuider'),
            ]);
        } catch (\Exception $e) {
            DB::rollback();
            return response()->json([
    ...
    private function getArrayValue($key, $array, $default = null)
    {
        $arrayCheck = is_array($array) && count($array) > 0 && array_key_exists($key, $array) && ! empty($array[$key]) && $array[$key] != 'null';
        return $arrayCheck ? $array[$key] : $default;
    }
    ...
    private function parseRequestData($flow, $flow_data): void
    {
        ...
        foreach ($flow_data['elements']['nodes'] as $node_data) {
            ...
            if ($node_data['type'] == 'starter-box') {
                ...
                $data['duration'] = $this->getArrayValue('text_duration', $box, 0);
                $connections['inputs'] = [];
                ...
                $data['duration'] = $this->getArrayValue('image_duration', $box, 0);
                ...
                $data['duration'] = $this->getArrayValue('audio_duration', $box, 0);
                $data['original_file_name'] = null;
                $data['mime_type'] = null;
                $connections['inputs'] = [];
                ...
                $data['duration'] = $this->getArrayValue('video_duration', $box, 0);
                $data['original_file_name'] = null;
                $data['mime_type'] = null;
                $connections['inputs'] = [];
                ...
                $data['type']     = 'document';
                $data['file']     = $box['file'];
                $data['duration'] = $this->getArrayValue('file_duration', $box, 0);
                $data['original_file_name'] = null;
                $data['mime_type'] = null;
                $connections['inputs'] = [];
                ...
                $data['duration'] = $this->getArrayValue('location_duration', $box, 0);
                $connections['inputs'] = [];
                $connections['outputs'][0]['type'] = 'location';
            } elseif ($node_data['type'] == 'box-with-condition') {
                $data['type'] = 'conditions';
                $data['conditions'] = $this->getArrayValue('condition_fields', $box, []);
                $connections['inputs'] = [];
                ...
                $data['duration'] = $this->getArrayValue('button_duration', $box, 0);
